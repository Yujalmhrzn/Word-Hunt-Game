...
function WordSearchAILogic(
  ...
  function getRandomWord() {
    ...
    let availableWords = processedList.filter(
      (word) => !wordFound.includes(word)
    );
    ...
    if (availableWords.length === 0) {
      ...
      if (AIcount > 10) {
        document.getElementById("gameInfo").textContent =
          "Oh no! Time called the shots and the AI left you in the dust!";
      }

      //if it is a draw
      else if (AIcount == 10) {
        document.getElementById("gameInfo").textContent =
          "DRAW! You and the AI shook hands and called it even!";
      }

      //if the user won
      else if (AIcount < 10) {
        document.getElementById("gameInfo").textContent =
          "VICTORY! You beat the clock! The AI did not stand a chance!";
      }
